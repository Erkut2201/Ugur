...
export default function InquiriesPage() {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <h1 className="text-2xl font-bold text-white mb-1">Anfragen</h1>
      ...
      {!isLoading && inquiries.length === 0 && (
        <div className="rounded-xl border border-gray-700 bg-gray-800/50 p-8 text-center text-gray-400 text-sm">
      ...
      {!isLoading && inquiries.length > 0 && (
        <div className="space-y-4">
          {inquiries.map((inquiry) => {
            ...
            return (
              <article key={inquiry.id} className="rounded-xl border border-gray-700 bg-gray-800 p-6">
                ...
                <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  <div>
                    ...
                    <dd className="mt-1 text-sm text-white">{inquiry.name}</dd>
                  </div>
                  <div>
                    <dt className="text-xs text-gray-500 uppercase tracking-wide">E-Mail</dt>
                    <dd className="mt-1 text-sm text-white">
                      <a href={`mailto:${inquiry.email}`} className="hover:text-brand-gold">{inquiry.email}</a>
                    </dd>
                  </div>
                  <div>
                    <dt className="text-xs text-gray-500 uppercase tracking-wide">Telefon</dt>
                    <dd className="mt-1 text-sm text-white">
                  ...
                  {inquiry.address && (
                    <div>
                      ...
                      <dd className="mt-1 text-sm text-white">{inquiry.address}</dd>
                    </div>
                  )}
                </div>

                <div className="mt-4">
                  <dt className="text-xs text-gray-500 uppercase tracking-wide">Nachricht</dt>
                  <dd className="mt-1 text-sm text-gray-200 whitespace-pre-wrap leading-6">{inquiry.message}</dd>
                </div>
                  ...
                  <div className="mt-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {files.map((file, index) => (
                        <span
                          key={index}
                          className="rounded-lg border border-gray-600 bg-gray-700 px-3 py-1.5 text-xs text-gray-200"
